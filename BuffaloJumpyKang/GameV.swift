...
struct GameV: View {
    @State var animationDo: Bool = false
    var body: some View {
        ZStack {
            Image("bgBuffalcik")
                .resizable()
        ...
        Image("logotipsik")
                .scaleEffect(animationDo ? 1 : 0.8)
                .onAppear() {
                    withAnimation(Animation.linear(duration: 1.2).repeatForever()) {
                        animationDo = true
                    }
                }
            WKWebViewRepresentable(url: URL(string: "https://www.mathplayground.com/jumpy_kangaroo/index.html")!, onLoadCompletion: {
                withAnimation {
